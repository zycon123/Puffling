...
## Critical test flow
1. Open `beta42.html` and confirm the Zycon Studios splash stays visible until startup is complete, shows **ORBUFF**, then every visible main-menu button responds on its first tap without a warm-up tap.
2. On a fresh profile choose exactly one of the 3 starter Orbuffs. Confirm Race My Orbuff unlocks after selection and the chooser cannot grant a second free starter.
3. Open OrbuffDex. Confirm all 100 Orbuffs are visible in the catalog, starters are identifiable, and the other 97 show evolution progression. Check a level-10 and level-20 Orbuff for Evolved/Ascended presentation.
...
7. On desktop (viewport >=900px with mouse/fine pointer), confirm the gameplay canvas is centered and capped near 620px wide, platforms are not spread across the full monitor, mouse steering maps correctly, and the Orbuff follower stays aligned with the player.
...
12. Open Nursery & Vault. Confirm all three egg tiers work, three fixed Vault slots are usable, and Vaulted Orbuffs are protected from Fusion.
13. Confirm **Starter Orbuff / Spark Orbuff / Drop Orbuff never hatch from Rare, Epic or Legendary eggs** and are not granted by boss rewards.
14. Test Fusion, Mystery Shop, Orbuff Treasure, achievements, cosmetics, upgrades and Daily Reward. Refresh and verify persistence.
15. Test Orbuff XP/evolution accumulation during a run, including small height increments.
16. Open Race My Orbuff. Confirm the old multiplayer HUD is not visible behind the Race HUD.
17. During Race, confirm YOU/GHOST meters and both progress bars update continuously rather than freezing.
18. Confirm the Race goal is 1500m, each Orbuff has max 3 attacks, and the normal minimum attack cooldown is 4 seconds.
...
24. Search the visible UI for the old word **Puffling**. It should not appear in menus, starter onboarding, OrbuffDex, Nursery/Vault, Race, toast messages, loading screens or share text.
25. Refresh an existing beta profile created before the rename. Inventory, active creature, Vault, eggs, currencies, starter choice and progression must survive unchanged and display as Orbuff.

## Startup recovery
- Interrupt one module request during startup. The same module should retry before later modules execute.
- Keep the request blocked. After three attempts a clear Orbuff retry screen must appear.
...
- `PFL-LAUNCH-101`: Race WebSocket endpoint is not configured. Deploy the existing Race server and set a production `wss://` URL before public online multiplayer launch.
...
Legacy `PFL-*`, `skyPuff*` and selected `Puffling*` internal identifiers remain intentionally during this beta so old saves and server/client contracts do not break. Player-facing branding must still be Orbuff everywhere.
