...
function openMultiplayer(){if(multiplayerMenuEl)multiplayerMenuEl.style.display='flex';if(multiplayerStatusEl)multiplayerStatusEl.textContent='Velg hvordan du vil spille';if(roomCodeDisplayEl)roomCodeDisplayEl.textContent='';}
...
function isBossUnlockedForRush(id){try{if(save&&Array.isArray(save.bosses)&&save.bosses.includes(id))return true;if(save&&Array.isArray(save.defeatedBosses)&&save.defeatedBosses.includes(id))return true;if(save&&save.bossWins&&save.bossWins[id])return true;}catch(e){}return false;}
function renderBossRush(){if(!bossRushListEl)return;bossRushListEl.innerHTML='';let any=false;bossRushBaseStages.forEach(stage=>{if(!isBossUnlockedForRush(stage.id))return;any=true;const b=document.createElement('button');b.className='secondary';b.style.margin='6px';b.innerHTML=`${stage.emoji} ${stage.name}<br><span style="font-size:12px">Reward: 50 🪙</span>`;b.onclick=()=>startBossRush(stage);bossRushListEl.appendChild(b);});if(!any)bossRushListEl.innerHTML='<div class="small">Beseir en boss i hovedspillet først for å låse den opp her.</div>';}
function openBossRush(){renderBossRush();if(bossRushMenuEl)bossRushMenuEl.style.display='flex';}
function closeBossRush(){if(multiplayerMenuEl)multiplayerMenuEl.style.display='none';if(multiplayerHudEl)multiplayerHudEl.style.display='none';if(bossRushMenuEl)bossRushMenuEl.style.display='none';}
function startBossRush(stage){closeBossRush();reset();bossRushMode=true;bossRushSelected={...stage,tier:1};paused=false;running=true;startEl.style.display='none';gameOverEl.style.display='none';missionCompleteEl.style.display='none';shopEl.style.display='none';upgradesEl.style.display='none';if(audioSettingsEl)audioSettingsEl.style.display='none';if(leaderboardMenuEl)leaderboardMenuEl.style.display='none';const realStage=bossStages.find(b=>b.id===stage.id);if(!realStage){running=false;showToast('Kunne ikke starte bossen');startEl.style.display='flex';return;}spawnBoss(realStage);lastTime=performance.now();requestAnimationFrame(loop);}
function finishBossRushWin(){const reward=50;save.bank=(save.bank||0)+reward;persist();refreshMenu();bossRushMode=false;bossRushSelected=null;boss=null;bossSpawned=false;bossArena=false;bossArenaY=0;playerShots=[];bossShots=[];running=false;paused=false;stopBossMusic(true);if(bossWrap)bossWrap.style.display='none';if(bossWarningEl)bossWarningEl.style.display='none';showToast(`Boss beseiret! +${reward} 🪙`);setTimeout(()=>{refreshMenu();startEl.style.display='flex';renderBossRush();},350);}
