(function(){
 ...
 function statusCard(label,state,detail){const ok=state==='ok',warn=state==='warn',info=state==='info';const icon=ok?'✅':warn?'⚠️':info?'ℹ️':'❌';return `<div style="background:rgba(255,255,255,.84);border:2px solid ${ok?'rgba(80,190,120,.25)':warn?'rgba(255,190,60,.32)':info?'rgba(70,155,230,.28)':'rgba(235,80,80,.28)'};border-radius:15px;padding:11px 13px"><div style="font-weight:1000;color:#35516b">${icon} ${label}</div><div style="font-size:12px;font-weight:800;color:#65788b;margin-top:4px;word-break:break-word">${detail}</div></div>`;}
 ...
 function codesFor(d){
   ...
   const publicLaunch=window.SKY_PUFF_PUBLIC_LAUNCH===true,nativePlatform=['ios','android'].includes(String(d.iap?.platform||'').toLowerCase());
   if(publicLaunch&&nativePlatform&&!d.iap?.canPurchase){const reason=!d.iap?'Diamond Store API mangler':!d.iap.bridgeReady?'Native App Store/Google Play-bro er ikke koblet til':!d.iap.verifyReady?'Serververifisering for kjøp er ikke konfigurert':'Betaling er ikke klar';codes.push({code:'PFL-LAUNCH-103',level:'launch',detail:reason});}
   if(publicLaunch&&!d.rankServerAuthoritative)codes.push({code:'PFL-LAUNCH-104',level:'launch',detail:'Ranked MMR/W/L er fortsatt lagret lokalt i nettleseren; offentlig ranked krever konto- og serverlagret profil'});
   if(publicLaunch&&!d.tradeServerInventory)codes.push({code:'PFL-LAUNCH-105',level:'launch',detail:'Trade-eierskap er fortsatt klientlokalt; offentlig trading krever server-autoritativ inventory'});
   return codes;
 }
 function render(){
