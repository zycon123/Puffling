/* Sky Puff — Diamond Mystery Shop v0.1 */
(function(){
 ...
 function grant(r){const F=window.SkyPuffFusion;
  ...
  else if(r.type==='egg'&&F){const base=Object.keys(F.BASE||{}),epic=['thunderflame','aurora','tempest'],leg=['eclipse','neonstorm'];let pool=r.tier==='legendary'?leg:r.tier==='epic'?epic:base;const id=pool[Math.floor(Math.random()*pool.length)];if(id)F.add(id,1);window.SkyPuffFusionUI?.renderDex?.();}
 ...
 function buy(){const out=document.getElementById('diamondBoxResult');if(!spend(COST)){if(out)out.textContent=`Du trenger ${COST} diamanter 💎`;return;}const r=roll();grant(r);if(out)out.textContent=`🎁 ${r.label}!`;if(typeof showToast==='function')showToast(`Mystery Box: ${r.label}!`);refresh();}
