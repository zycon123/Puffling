...
(function(){
 ...
 function render(){
  ...
  if(progress)progress.innerHTML=`<b>OrbVault Level ${state.level}/5</b><br>${tr('vaultSlotsLine',{slots:cfg.slots,min:cfg.energyMinutes,xph:cfg.xpHour})}<br>Revive Orbs: <b>${state.reviveOrbs}</b>${tr(state.level<5?'vaultBonusPending':'vaultBonusActive')}`;
  const filled=slots.filter(Boolean);if(!filled.length){host.innerHTML=`<div class="orbVaultLiveEmpty">${tr('vaultEmptyHint')}</div>`;return}
  host.innerHTML=filled.map(id=>{const q=puff(id),energy=E()?.get?.(id),xp=X()?.get?.(id),rest=state.rest?.[id],energyFull=energy&&energy.energy>=energy.max,nextEnergy=rest&&!energyFull?(+rest.energyAt||now)+cfg.energyMinutes*60000-now:0,nextXp=rest?(+rest.xpAt||now)+3600000-now:0;return `<div class="orbVaultLiveRow"><div class="orbVaultLiveTop"><span>${q?.icon||'☁️'} ${q?.name||id}</span><span>❤️ ${energy?.energy??'?'} / ${energy?.max??'?'}</span></div><div class="orbVaultLiveMeta">Level ${xp?.level||1} • ${energyFull?tr('vaultFullEnergy'):tr('vaultNextEnergyIn',{time:fmt(nextEnergy)})} • ${tr('vaultNextXpIn',{xph:cfg.xpHour,time:fmt(nextXp)})}</div></div>`}).join('')
 }
