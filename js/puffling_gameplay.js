/* Orbuff — gameplay integration v0.3 (legacy Puffling identifiers retained) */
(function(){
  ...
  function refreshHud(){ensureHud();const d=document.getElementById('activePufflingHud'),p=getPuff(active());if(!d)return;d.innerHTML=p?`${p.icon||'☁️'} ${p.name}`:tr('noActivePuffling');d.style.display=(typeof running!=='undefined'&&running)?'block':'none'}
