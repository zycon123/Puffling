/* Sky Puff — first Puffling starter choice v1.0 */
(function(){
 ...
 function eligible(){return !ownsAny()&&!chosen()}
 function ensure(){
 ...
 function render(){
  ...
  IDS.forEach(id=>{const p=puff(id);if(!p)return;const b=document.createElement('button');b.type='button';b.className='secondary';b.style.cssText='min-width:0;margin:0;padding:14px 8px;min-height:150px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;border:2px solid rgba(80,160,210,.22)';b.innerHTML=`<span style="font-size:38px">${p.icon||'☁️'}</span><b style="font-size:15px">${p.name}</b><small style="font-weight:1000;opacity:.7">${t.common}</small><small style="opacity:.68">${t.weak}</small><span style="margin-top:5px;font-weight:1000">${t.choose}</span>`;b.onclick=()=>select(id);grid.appendChild(b);});
 ...
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(maybeShow,120));else setTimeout(maybeShow,120);
 document.getElementById('languageSelect')?.addEventListener('change',()=>{if(ensure().style.display!=='none')render()});
 window.SkyPuffStarterChoice={IDS,ownsAny,chosen,eligible,open,select,render};
})();
