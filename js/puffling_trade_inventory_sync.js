...
(function(){
 ...
 function fail(){const e=document.getElementById('tradeStatus');if(e){e.textContent='Kunne ikke synkronisere Orbuff-samlingen med serveren. Trade er stoppet for å beskytte inventory.';e.style.color='#a73535';}}
 function install(){const t=window.PufflingTrade;if(!t||t.__inventorySyncInstalled)return false;const original=t.connect.bind(t);t.connect=async function(code){const e=document.getElementById('tradeStatus');if(e)e.textContent='Synkroniserer sikker Orbuff-samling…';try{await ensure();return original(code);}catch(err){fail();return false;}};t.ensureServerInventory=ensure;t.__inventorySyncInstalled=true;
