(function(){
  ...
  const requiredApis=['skyPuffAchievements','skyPuffAchievementsMenu','skyPuffBetaDiagnostics','skyPuffAIDiagnostics','skyPuffAntiCheat','skyPuffDiagnosticsSupport','skyPuffPlatform','SkyPuffFusion','SkyPuffFusionUI','SkyPuffPufflingGameplay','SkyPuffPufflingProgress','SkyPuffPufflingEvolution','SkyPuffPerformance','SkyPuffPerformanceUI','SkyPuffBossPufflingRewards','SkyPuffRace','SkyPuffRaceUI','SkyPuffRaceTransport','SkyPuffRaceNetwork','SkyPuffRaceProgress','SkyPuffQuickRank','SkyPuffNurseryVault','SkyPuffDiamonds','SkyPuffMysteryShop','PufflingDiamondStore','SkyPuffUniqueTraits','PufflingBossRush','PufflingBossPersistence','PufflingRaceMode','PufflingTrade','PufflingBrand','PufflingExtraCollection','PufflingExpansion36'];
  ...
  try{const I=window.PufflingDiamondStore,c=I?.catalog?.()||[];if(c.length!==5)missing.push('iap:expected-5-products');if(c.some(x=>!/^puffling\.diamonds\./.test(x.id)||!Number.isFinite(Number(x.diamonds))))missing.push('iap:invalid-product-catalog');if(typeof I?.purchase!=='function'||typeof I?.status!=='function')missing.push('iap:store-api-missing');}catch(e){missing.push('iap:check-failed');}
