...
for(const token of ['SkyPuffRace','SkyPuffRaceProgress','SkyPuffQuickRank','PufflingDiamondWallet','PufflingDiamondStore','expected-100-total','PufflingExpansion36','SkyPuffUniqueTraits'])if(!smoke.includes(token))fail(`Smoke Check missing current launch invariant: ${token}`);
if(!process.exitCode)ok('Smoke Check covers current Race, rank, wallet, IAP and 100-Puffling systems');
...
for(const code of ['PFL-SMOKE-001','PFL-SAVE-001','PFL-RUNTIME-001','PFL-AC-001','PFL-AI-001','PFL-LAUNCH-101','PFL-LAUNCH-102','PFL-LAUNCH-103'])if(!diagnostics.includes(code))fail(`Diagnostics missing code ${code}`);
if(!process.exitCode)ok('System & Support exposes actionable error and launch codes');

for(const token of ['name: puffling-race-server','rootDir: server','healthCheckPath: /health','autoDeploy: true'])if(!render.includes(token))fail(`Render blueprint missing: ${token}`);
...
for(const token of ['puffling_wallets','puffling_iap_transactions','puffling_wallet_ledger','client_key_hash','paid_diamonds','provider_not_configured','insufficient_paid_diamonds'])if(!iapServer.includes(token))fail(`Diamond wallet server missing invariant: ${token}`);
...
if(!bootstrap.includes("require('./iap_store')")||!bootstrap.includes("require('./index')"))fail('Server bootstrap does not attach IAP wallet to multiplayer service');
else ok('Persistent paid-Diamond wallet and verifier API are wired fail-closed');
...
if(!beta.includes("SKY_PUFF_IAP_VERIFY_URL=SKY_PUFF_GAME_API_URL+'/iap/verify'"))fail('IAP verifier route is not configured to the production API');
else ok('Wallet and IAP verifier endpoints are configured; provider verification remains fail-closed server-side');
