...
if(!process.exitCode)ok('System & Support exposes actionable runtime and launch codes');
...
for(const token of ['rankRating','race:positionRejected','HEIGHT_RATE_LIMIT','HEIGHT_START_ALLOWANCE','integrityLocked','integrityVersion'])if(!raceServer.includes(token))fail(`Race server missing launch integrity invariant: ${token}`);
if(!process.exitCode)ok('Race server relays MMR and rejects impossible movement/finish teleports');
