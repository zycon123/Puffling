...
if(!await M.buyBoxes(1)||D.get()!==975||M.boxCount()!==1)fail('1-box purchase did not store unopened box');
if(!await M.buyBoxes(3)||D.get()!==900||M.boxCount()!==4)fail('3-box purchase failed');
if(!await M.buyBoxes(10)||D.get()!==650||M.boxCount()!==14)fail('10-box purchase failed');
...
if(typeof D.earned!=='function'||typeof D.paid!=='function')fail('earned/paid Diamond split API missing');
console.log('✅ Mystery Box vault, bundles, earned/paid split and 3→1 egg fusion validated');
