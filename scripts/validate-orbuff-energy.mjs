...
let active='ember',stage=0,persisted=0,bossStarts=0;
...
const ctx={console,Date,JSON,Object,Number,String,Math,localStorage,running:false,save:{bank:1000},persist:()=>persisted++,refreshMenu:()=>{},showToast:()=>{},playBtnEl:null,retryBtnEl:null,startGame(){ctx.running=true;return true},startBossRush(){bossStarts++;ctx.running=true;return true},endGame(){ctx.running=false},SkyPuffPufflingEvolution:{stageFor:()=>stage},SkyPuffFusion:{BASE:puffs,FUSIONS:{}},SkyPuffFusionUI:{renderDex:()=>{}},SkyPuffPufflingGameplay:{active:()=>active,getPuff:id=>puffs[id],setActive:id=>{active=id;return true}}};
...
console.log('✅ Orbuff energy validated: 5 base, 6 Ascended, exhaustion, revival and Boss Rush loss/start guards');