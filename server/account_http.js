...
module.exports=function createAccountHttp(auth,rankedStore,accountStore){
  return async function handleAccount(req,res){
    ...
    if(url.pathname==='/api/account/status'&&req.method==='GET')return json(res,200,{ok:true,enabled:auth.enabled(),issuer:auth.issuer,audience:auth.audience,ttlSeconds:auth.ttlSeconds,deletionReady:!!accountStore?.status?.().ready});
    ...
    if(url.pathname==='/api/account/me'&&req.method==='GET'){
      ...
      return json(res,200,{ok:true,accountId:verified.accountId,kind:String(verified.claims?.kind||'guest'),profile,serverAuthoritative:true,deletionReady:!!accountStore?.status?.().ready});
    }
