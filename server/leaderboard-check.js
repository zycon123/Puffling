...
(async()=>{
  ...
  if(submitted.name!=='Alice'||submitted.height!==1234||submitted.accountLinked!==true)fail('Score normalization/account linkage failed');
  const insert=calls.find(x=>/^INSERT INTO puffling_scores/i.test(x.sql.trim()));if(!insert||insert.args[0]!=='Alice'||insert.args[1]!==1234||insert.args[2]!=='abcdef12'||insert.args[4]!=='guest_test_account')fail('Normalized linked score was not written');
  ...
  console.log('✅ Persistent leaderboard normalization, authenticated account linkage and listing passed');
