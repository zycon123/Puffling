...
(async()=>{
 ...
 const store={status:()=>({ready:true}),snapshot:async id=>{assert.equal(id,'acct_a');return inventory;},migrateLegacy:async(id,data)=>{assert.equal(id,'acct_a');assert.deepEqual(data,{owned:{ember:2,shadow:1},vault:['shadow']});if(migrated)return{applied:false,duplicate:true,version:1};migrated=true;return{applied:true,duplicate:false,version:1,count:2,protectedCount:1};}};
